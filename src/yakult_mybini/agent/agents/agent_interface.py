...
class AgentInterface(ABC):
    ...
    def set_memory_manager(self, memory_manager) -> None:
        """Set the long-term memory manager for this agent."""
        self._memory_manager = memory_manager

    def set_todo_manager(self, todo_manager) -> None:
        """Set the todo/reminder manager for this agent."""
        self._todo_manager = todo_manager

    def set_conf_uid(self, conf_uid: str) -> None:
        """Set the configuration UID for this agent session.

        This is used for loading/storing long-term memories specific to this configuration.
        Should be called during initial connection setup.
    ...
    def get_memory_prompt(self) -> str:
        """Get long-term memory context string for injection into system prompt."""
        if self._memory_manager:
            return self._memory_manager.to_prompt_string()
        return ""
