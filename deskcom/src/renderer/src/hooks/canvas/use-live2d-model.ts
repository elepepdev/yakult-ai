...
export const useLive2DModel = ({
  ...
  const handleMouseEnter = useCallback((e: React.MouseEvent) => {
    if (!isPet || getPlatform().name === 'web') return;
    const adapter = (window as any).getLAppAdapter?.();
    const view = LAppDelegate.getInstance().getView();
    const model = adapter?.getModel();
    if (!adapter || !view || !model || !canvasRef.current) return;
    ...
    const isHit = model.anyhitTest(modelX, modelY) !== null || model.isHitOnModel(modelX, modelY);
    if (isHit !== isHoveringModelRef.current) {
      isHoveringModelRef.current = isHit;
      getPlatform().updateComponentHover('live2d-model', isHit);
    }
  }, [isPet, canvasRef]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isPet && !isDragging && !isPotentialTapRef.current && getPlatform().name !== 'web' && adapter && view && model && canvasRef.current) {
      const canvas = canvasRef.current;
      const rect = canvas.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      ...
      if (currentHitState !== isHoveringModelRef.current) {
        ...
        getPlatform().updateComponentHover('live2d-model', currentHitState);
      }
    }
    // --- End Pet Hover Logic ---
  }, [isPet, isDragging, canvasRef]);
  ...
  const handleMouseLeave = useCallback(() => {
    ...
    // --- Pet Hover Logic (Unchanged) ---
    if (isPet && getPlatform().name !== 'web' && isHoveringModelRef.current) {
      isHoveringModelRef.current = false;
      getPlatform().updateComponentHover('live2d-model', false);
    }
  }, [isPet, isDragging, handleMouseUp]);
