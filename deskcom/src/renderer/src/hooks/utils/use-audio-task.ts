  ...
  const handleAudioPlayback = (options: AudioTaskOptions): Promise<void> => new Promise((resolve) => {
    ...
    try {
      ...
      if (audioBase64) {
        ...
        // Get Live2D manager and model (optional — unavailable in VRM mode)
        const live2dManager = (window as any).getLive2DManager?.();
        const model = live2dManager?.getModel(0) ?? null;

        if (model) {
          console.log('Found Live2D model for audio playback');
          if (!model._wavFileHandler) {
            console.warn('Model does not have _wavFileHandler for lip sync');
          }
        } else {
          console.log('No Live2D model (VRM mode) — audio-only playback');
        }

        // Set expression for Live2D (only when Live2D is active)
        ...
        // Dispatch expression & volume event for VRM (non-invasive, Live2D unaffected)
        const vrmEvent = new CustomEvent('vrm-audio-task', {
          detail: {
            expressions: expressions || [],
            volumes: options.volumes || [],
            sliceLength: options.sliceLength || 20,
          },
        });
        window.dispatchEvent(vrmEvent);

        // Start talk motion (Live2D only)
        ...
        const cleanup = () => {
          ...
          // Notify VRM to stop lip-sync
          window.dispatchEvent(new CustomEvent('vrm-lip-sync-stop'));
          if (!isFinished) {
            isFinished = true;
            resolve();
          }
        ...
        audio.addEventListener('canplaythrough', () => {
          ...
          // Dispatch VRM lip-sync start event with audio element reference
          // This lets VRM track audio.currentTime for per-frame viseme updates
          window.dispatchEvent(new CustomEvent('vrm-lip-sync-start', {
            detail: {
              audio,
              volumes: options.volumes || [],
              sliceLength: options.sliceLength || 20,
          ...
          // Setup lip sync (Live2D only — model is null in VRM mode)
          if (model?._wavFileHandler) {
