...
var armTime = 1.5;
var spread = 0.19 + Math.sin(armTime * Math.PI * 2 / 6) * 0.09;
E.set(0, 0, spread);
Q.setFromEuler(E);
...
console.log("head world:", hp.x.toFixed(4), hp.y.toFixed(4), hp.z.toFixed(4));
var cam = new THREE3.PerspectiveCamera(20, SW / SH, 0.1, 100);
cam.position.set(hp.x, hp.y, 3);
cam.lookAt(hp.x, hp.y, hp.z);
cam.updateMatrixWorld(true);
cam.updateProjectionMatrix();
console.log("cam pos:", cam.position.x.toFixed(4), cam.position.y.toFixed(4), cam.position.z.toFixed(4));
var cx = (x) => (x + 1) / 2 * SW;
var SHp = hp.clone().project(cam);
console.log("head screen-x:", cx(SHp.x).toFixed(1));
for (const k of ["LH", "RH"]) {
  const P = B[k].getWorldPosition(new THREE3.Vector3());
  const S = P.clone().project(cam);
  console.log(`${k}: world(${P.x.toFixed(3)},${P.y.toFixed(3)},${P.z.toFixed(3)})  screen-x ${cx(S.x).toFixed(1)}  screen-y ${((1 - S.y) / 2 * SH).toFixed(1)}`);
}
var f = 1 / Math.tan(10 * Math.PI / 180);
var aspect = SW / SH;
for (const k of ["LH", "RH"]) {
  const P = B[k].getWorldPosition(new THREE3.Vector3());
  const dx = P.x - hp.x, dy = P.y - hp.y, zeye = cam.position.z - P.z;
  const xn = f * dx / zeye / aspect;
  const yn = f * dy / zeye;
  console.log(`manual ${k}: x_ndc ${xn.toFixed(3)} -> screen ${cx(xn).toFixed(1)} | y_ndc ${yn.toFixed(3)}`);
}
