...
function mirrorW(q, rootQ) {
  const M = new THREE3.Matrix4().makeScale(-1, 1, 1);
  const rootM = new THREE3.Matrix4().makeRotationFromQuaternion(rootQ);
  const rootMi = new THREE3.Matrix4().makeRotationFromQuaternion(rootQ.clone().invert());
  const MW = new THREE3.Matrix4().multiplyMatrices(rootM, M).multiply(rootMi);
...
function run(mode, seconds) {
  ...
  for (let f = 0; f < seconds / dt; f++) {
    ...
    if (mode === "bakedOnly") {
      ...
      } else if (mode === "mirrorChain" || mode === "mirrorUpper") {
        vrm.scene.updateMatrixWorld(true);
        const rootQ = B.hips.getWorldQuaternion(new THREE3.Quaternion());
        const mk = (pR, pL, qL) => {
          const wQ = new THREE3.Quaternion().multiplyQuaternions(pL.getWorldQuaternion(new THREE3.Quaternion()), qL);
          const mQ = mirrorW(wQ, rootQ);
          return pR.getWorldQuaternion(new THREE3.Quaternion()).invert().multiply(mQ);
        };
        B.RU.quaternion.copy(mk(B.RU, B.LU, B.LU.quaternion));
        B.RU.quaternion.copy(mk(B.RU, B.LU, B.LU.quaternion));
        if (mode === "mirrorChain") {
          B.RL.quaternion.copy(mk(B.RL, B.LL, B.LL.quaternion));
          B.RH.quaternion.copy(mk(B.RH, B.LH, B.LH.quaternion));
        }
      }
    }
    vrm.update(dt);
