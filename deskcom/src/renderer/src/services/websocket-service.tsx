...
export interface Message {
  ...
  type?: 'text' | 'tool_call_status' | 'youtube-invite' | 'music-play-song';
  ...
  progress?: number | null; // 0-100 percentage, null = indeterminate
  output_lines?: string[]; // Accumulated output lines for command tools

  // Fields for youtube-invite
  stream_url?: string;
  request_id?: string;
  video_url?: string;
}
...
class WebSocketService {
  ...
  private initializeConnection() {
    ...
    const savedGridSpec = typeof window !== 'undefined' ? localStorage.getItem('gridSpec') : null;
    if (savedGridSpec) {
      try {
        const parsed = JSON.parse(savedGridSpec);
        this.sendMessage({ type: 'set-grid-spec', grid_spec: parsed });
      } catch {
        this.sendMessage({ type: 'set-grid-spec', grid_spec: savedGridSpec });
      }
    }
  }
  ...
  connect(url: string) {
    ...
    const platform = typeof window !== 'undefined' && (window as any).api !== undefined ? 'electron' : 'web';
    const separator = url.includes('?') ? '&' : '?';
    const platformUrl = `${url}${separator}platform=${platform}`;
