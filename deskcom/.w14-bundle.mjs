...
function run(mode, seconds) {
  ...
  for (let f = 0; f < seconds / dt; f++) {
    ...
    if (mode === "mirrorArm") {
      ...
      const c = (wL.x + wR.x) / 2;
      maxMir.push(Math.hypot(wR.x - (2 * c - wL.x), wR.y - wL.y, wR.z - wL.z));
    }
    if (f > 0.5 / dt) {
      Ls.push(xl);
      Rs.push(xr);
  ...
  console.log(`${mode.padEnd(12)}: Lx ${stat(Ls)}  Rx ${stat(Rs)} | Ly ${ys(Ly)} Ry ${ys(Ry)}${mode === "mirrorArm" ? ` | hand mirror err ${Math.max(...maxMir).toFixed(4)}m` : ""}`);
}
run("bakedOnly", 6.5);
run("raw", 6.5);
run("mirrorArm", 6.5);
