...
var SH = 882;
var cx = (x) => (x + 1) / 2 * SW;
function measure(mirrorRight, label) {
  mixer.setTime(2);
  mixer.update(0);
  vrm.update(0);
  vrm.scene.updateMatrixWorld(true);
  const lb = B.LU.quaternion.clone(), rb = B.RU.quaternion.clone();
  if (mirrorRight) {
    rb.set(-lb.x, lb.y, lb.z, lb.w);
  }
  ...
  let Lc = 0, Rc = 0, n = 0, maxL = 0, maxR = 0, minL = 1e9, minR = 1e9;
  let armTime = 0;
  for (let f = 0; f < 6 / dt; f++) {
    ...
    Rc += xr;
    n++;
    maxL = Math.max(maxL, Math.abs(xl - 451));
    minL = Math.min(minL, Math.abs(xl - 451));
    maxR = Math.max(maxR, Math.abs(xr - 451));
    minR = Math.min(minR, Math.abs(xr - 451));
  }
  console.log(`${label}:`);
  console.log(`  screen-x center: L ${(Lc / n).toFixed(0)} R ${(Rc / n).toFixed(0)} | dist-from-center L ${minL.toFixed(0)}-${maxL.toFixed(0)}px R ${minR.toFixed(0)}-${maxR.toFixed(0)}px`);
}
measure(false, "RAW bases (asymmetric)");
measure(true, "MIRRORED left base -> right");
