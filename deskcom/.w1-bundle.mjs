...
var SW = 902;
var SH = 882;
function setupCam(kScale2) {
  vrm.update(0);
  vrm.scene.updateMatrixWorld(true);
  const hp = B.head.getWorldPosition(new THREE3.Vector3());
  const cam2 = new THREE3.PerspectiveCamera(20, SW / SH, 0.1, 100);
  cam2.position.set(hp.x, hp.y, 3 / kScale2);
  cam2.lookAt(hp.x, hp.y, hp.z);
  cam2.updateMatrixWorld(true);
  cam2.updateProjectionMatrix();
  return cam2;
}
var kScale = 1;
var cam = setupCam(kScale);
mixer.setTime(2);
mixer.update(0);
...
var RwxMin = 1e9;
var RwxMax = -1e9;
var HEADX = B.head.getWorldPosition(new THREE3.Vector3()).x;
for (let f = 0; f < 6 / dt; f++) {
  ...
  const cx = (x) => (x + 1) / 2 * SW;
  LxMin = Math.min(LxMin, cx(SL.x));
  LxMax = Math.max(LxMax, cx(SL.x));
  RxMin = Math.min(RxMin, cx(SR.x));
  RxMax = Math.max(RxMax, cx(SR.x));
  LyMin = Math.min(LyMin, cx(SL.y));
  LyMax = Math.max(LyMax, cx(SL.y));
  RyMin = Math.min(RyMin, cx(SR.y));
  RyMax = Math.max(RyMax, cx(SR.y));
  LwxMin = Math.min(LwxMin, PL.x - HEADX);
  LwxMax = Math.max(LwxMax, PL.x - HEADX);
  RwxMin = Math.min(RwxMin, PR.x - HEADX);
  RwxMax = Math.max(RwxMax, PR.x - HEADX);
}
console.log(`kScale=${kScale} (app camera, head-level)`);
console.log(`SCREEN-X travel  LEFT: ${LxMin.toFixed(0)}-${LxMax.toFixed(0)}px (width ${(LxMax - LxMin).toFixed(0)})  RIGHT: ${RxMin.toFixed(0)}-${RxMax.toFixed(0)}px (width ${(RxMax - RxMin).toFixed(0)})`);
console.log(`SCREEN-Y travel  LEFT: ${LyMin.toFixed(0)}-${LyMax.toFixed(0)}px (${(LyMax - LyMin).toFixed(0)})  RIGHT: ${RyMin.toFixed(0)}-${RyMax.toFixed(0)}px (${(RyMax - RyMin).toFixed(0)})`);
console.log(`WORLD |x-head| travel  LEFT: ${(LwxMin * 1e3).toFixed(0)}..${(LwxMax * 1e3).toFixed(0)}mm (${((LwxMax - LwxMin) * 1e3).toFixed(0)}mm)  RIGHT: ${(RwxMin * 1e3).toFixed(0)}..${(RwxMax * 1e3).toFixed(0)}mm (${((RwxMax - RwxMin) * 1e3).toFixed(0)}mm)`);
