        ...
        for tool_name, data_object in formatted_tools_dict.items():
            ...
            for param_name, param_info in properties.items():
                ...
                if param_schema["type"] == "array":
                    raw_items = param_info.get("items")
                    if raw_items and raw_items.get("type"):
                        param_schema["items"] = raw_items
                    else:
                        logger.warning(
                            f"MC: Array parameter '{param_name}' in tool '{tool_name}' has missing or empty 'items'. Assuming items are strings."
                        )
