...
var SH = 882;
var cx = (x) => (x + 1) / 2 * SW;
var mirror = (q) => new THREE3.Quaternion(-q.x, q.y, q.z, q.w);
function run(mode) {
  mixer.setTime(0);
  mixer.update(0);
  let armSpreadTime = 0, ramp = 0;
  let leftBase = null, rightBase = null;
  ...
  for (let f = 0; f < 20 / dt; f++) {
    ...
    } else if (mode === "mirrorBase") {
      E.set(0, 0, -spread);
      Q.setFromEuler(E);
      B.RU.quaternion.copy(mirror(leftBase)).multiply(Q);
    } else if (mode === "mirrorUpper") {
      B.RU.quaternion.copy(mirror(B.LU.quaternion));
    } else if (mode === "mirrorChain") {
      B.RU.quaternion.copy(mirror(B.LU.quaternion));
      B.RL.quaternion.copy(mirror(B.LL.quaternion));
      B.RH.quaternion.copy(mirror(B.LH.quaternion));
    }
    vrm.update(dt);
    ...
    const d = Math.abs(xl - 451 - (451 - xr));
    sumAbs += d;
    n++;
    maxAbs = Math.max(maxAbs, d);
  }
  console.log(`${mode.padEnd(12)}: avg|asym| ${(sumAbs / n).toFixed(1)}px  max ${maxAbs.toFixed(1)}px`);
}
run("raw");
run("mirrorBase");
run("mirrorUpper");
