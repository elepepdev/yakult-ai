...
var w = new THREE3.Vector3();
var w2 = new THREE3.Vector3();
function pr(tag) {
  const wL = B.LH.getWorldPosition(w), wR = B.RH.getWorldPosition(w2);
  const hx = B.hips.getWorldPosition(new THREE3.Vector3()).x;
  const cx = B.head.getWorldPosition(new THREE3.Vector3()).x;
  console.log(`t=${tag} L=(${wL.x.toFixed(3)},${wL.z.toFixed(3)}) R=(${wR.x.toFixed(3)},${wR.z.toFixed(3)}) | Ldist(cx)=${((wL.x - cx) * 1e3).toFixed(0)}mm Rdist(cx)=${((wR.x - cx) * 1e3).toFixed(0)}mm | hips.x=${hx.toFixed(3)} head.x=${cx.toFixed(3)}`);
}
mixer.setTime(0);
mixer.update(0);
var leftBase = null;
var armSpreadTime = 0;
var spreadAt = 1.5;
for (let f = 0; f < spreadAt / dt; f++) {
  mixer.update(dt);
  if (!leftBase) leftBase = B.LU.quaternion.clone();
  armSpreadTime += dt;
  const spread = 0.19 + Math.sin(armSpreadTime * Math.PI * 2 / 6) * 0.09;
  E.set(0, 0, spread);
  Q.setFromEuler(E);
