...
export const useLive2DResize = ({
  ...
  const handleResize = useCallback(() => {
    ...
    try {
      ...
      const { width, height } = containerBounds || { width: window.innerWidth, height: window.innerHeight };
      ...
      if (!containerBounds) {
        console.warn('[Resize] Container bounds not available, falling back to window dimensions.');
      }
      if (width === 0 || height === 0) {
