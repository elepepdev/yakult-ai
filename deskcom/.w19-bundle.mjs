...
var j = vrm.springBoneManager?.joints;
if (j) {
  for (const jt of j) {
    const b = jt.bone;
    if (b && /arm|hand|wrist|finger/i.test(b.name)) console.log("SPRING joint on:", b.name);
  }
}
vrm.update(0.016);
...
function run(withSpring, seconds) {
  mixer.setTime(0);
  mixer.update(0);
  ...
  const zs = (a) => `z ${Math.min(...a).toFixed(3)}..${Math.max(...a).toFixed(3)}`;
  console.log(`${withSpring ? "spring" : "nospring"}: Lx ${stat(Lx)}  Rx ${stat(Rx)} | L${zs(Lw)} R${zs(Rw)}`);
}
run(true, 8.5);
run(false, 8.5);
