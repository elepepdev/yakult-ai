...
interface ChatHistoryState {
  ...
  appendYoutubeInvite: (data: { title: string; stream_url: string; request_id: string }) => void;
...
export function ChatHistoryProvider({ children }: { children: React.ReactNode }) {
  ...
  const appendOrUpdateToolCallMessage = useCallback((toolMessageData: Partial<Message>) => {
    ...
    setMessages((prevMessages) => {
      ...
      if (existingMessageIndex !== -1) {
        ...
        const isCommand = toolMessageData.tool_name === 'run_command' || toolMessageData.tool_name === 'run_sudo_command';
        const newLines: string[] = [];
        if (isCommand && toolMessageData.content && toolMessageData.status === 'running') {
          const line = toolMessageData.content.replace(/^\[(stdout|stderr)\] /, '');
        ...
        updatedMessages[existingMessageIndex] = {
          ...
          content: isCommand && toolMessageData.status === 'completed' || toolMessageData.status === 'error'
            ? (toolMessageData.content || existingMsg.content)
            : (existingMsg.content || toolMessageData.content || ''),
          timestamp: toolMessageData.timestamp!,
          progress: toolMessageData.progress !== undefined ? toolMessageData.progress : existingMsg.progress,
          output_lines: newLines.length > 0
            ? [...(existingMsg.output_lines || []), ...newLines]
            : (existingMsg.output_lines || []),
        };
        ...
        if (isCommand && toolMessageData.content && toolMessageData.status === 'running') {
          const line = toolMessageData.content.replace(/^\[(stdout|stderr)\] /, '');
          if (line && !line.startsWith('$ ')) initialLines.push(line);
        }
  ...
  const appendYoutubeInvite = useCallback((data: { title: string; stream_url: string; request_id: string }) => {
    const newMessage: Message = {
      id: data.request_id,
      content: data.title,
      role: 'ai',
