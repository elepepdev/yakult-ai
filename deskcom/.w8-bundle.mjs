...
var SH = 882;
var cx = (x) => (x + 1) / 2 * SW;
var M = new THREE3.Matrix4().makeScale(-1, 1, 1);
var mirrorWorldQ = (q) => new THREE3.Quaternion().setFromRotationMatrix(new THREE3.Matrix4().multiplyMatrices(M, new THREE3.Matrix4().makeRotationFromQuaternion(q)).multiply(M));
var tmp = new THREE3.Quaternion();
function makeCam() {
...
function mirrorChain() {
  const mk = (pL, pR, qL) => {
    ...
    const mQ = mirrorWorldQ(wQ);
    return pR.getWorldQuaternion(new THREE3.Quaternion()).invert().multiply(mQ);
  };
  B.RU.quaternion.copy(mk(B.LS, B.RS, B.LU.quaternion));
  B.RL.quaternion.copy(mk(B.LU, B.RU, B.LL.quaternion));
  B.RH.quaternion.copy(mk(B.LL, B.RL, B.LH.quaternion));
}
function run(mode, seconds) {
  mixer.setTime(0);
  mixer.update(0);
  const cam = makeCam();
  let leftBase = null, armSpreadTime = 0, ramp = 0;
  const Ls = [], Rs = [], dAs = [];
  for (let f = 0; f < seconds / dt; f++) {
    ...
    if (mode === "bakedOnly") {
      ...
      if (mode === "raw") {
        E.set(0, 0, -spread);
        Q.setFromEuler(E);
        B.RU.quaternion.copy(B.RU.quaternion).multiply(Q);
      } else if (mode === "gain") {
        E.set(0, 0, -spread * 0.52);
        Q.setFromEuler(E);
        B.RU.quaternion.copy(B.RU.quaternion).multiply(Q);
      } else if (mode === "mirrorChain") {
        mirrorChain();
      } else if (mode === "commonAxis") {
        const cW = B.chest.getWorldQuaternion(new THREE3.Quaternion());
        const rW = new THREE3.Quaternion().multiplyQuaternions(cW, new THREE3.Quaternion().setFromEuler(new THREE3.Euler(0, 0, spread)));
        const rM = mirrorWorldQ(rW);
        B.RU.quaternion.copy(tmp.copy(B.LU.quaternion));
        const pL = B.LS.getWorldQuaternion(new THREE3.Quaternion());
        const pR = B.RS.getWorldQuaternion(new THREE3.Quaternion());
        B.LU.quaternion.copy(pL.clone().invert().multiply(rW).multiply(pL).multiply(leftBase));
        B.RU.quaternion.copy(pR.clone().invert().multiply(rM).multiply(pR).multiply(B.RU.quaternion));
      }
    }
    vrm.update(dt);
  ...
  console.log(`${mode.padEnd(12)}: L ${stat(Ls)}  R ${stat(Rs)}  avg|asym| ${(dAs.reduce((a, b) => a + b, 0) / dAs.length).toFixed(0)}px`);
}
run("bakedOnly", 6.5);
run("raw", 6.5);
run("gain", 6.5);
