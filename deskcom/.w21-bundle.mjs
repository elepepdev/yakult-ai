...
function run(trackCam, seconds) {
  ...
  const Lx = [], Rx = [], Ly = [], Ry = [], dA = [];
  const v = new THREE3.Vector3(), v2 = new THREE3.Vector3(), v3 = new THREE3.Vector3();
  for (let f = 0; f < seconds / dt; f++) {
    ...
    const spread = (0.1 + Math.sin(armSpreadTime * Math.PI * 2 / 8) * 0.075) * ramp;
    E.set(0, 0, spread);
    Q.setFromEuler(E);
    ...
    vrm.scene.updateMatrixWorld(true);
    if (trackCam) {
      const c = (B.LS.getWorldPosition(v3).x + B.RS.getWorldPosition(new THREE3.Vector3()).x) / 2;
      cam.position.x = c;
      cam.lookAt(c, cam.position.y, 0);
      cam.updateMatrixWorld(true);
      cam.updateProjectionMatrix();
    }
    const SL = B.LH.getWorldPosition(v).project(cam);
    const SR = B.RH.getWorldPosition(v2).project(cam);
    ...
    if (f > 0.5 / dt) {
      ...
      Ly.push(yl);
      Ry.push(yr);
      dA.push(Math.abs(xl - 451 + (xr - 451)));
    }
  }
  const stat = (a) => {
  ...
  console.log(`${trackCam ? "track" : "fixed"}: Lx ${stat(Lx)}  Rx ${stat(Rx)} | Ly ${stat(Ly)}  Ry ${stat(Ry)} | avg|Ldist+Rdist| ${(dA.reduce((a, b) => a + b, 0) / dA.length).toFixed(1)}px`);
}
run(false, 8.5);
run(true, 8.5);
