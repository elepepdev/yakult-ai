...
class OpenAICompatibleConfig(StatelessLLMBaseConfig):
    ...
    _OPENAI_COMPATIBLE_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        ...
        "max_tokens": Description(
            en="Maximum tokens in the response. None = provider default.",
            zh="响应中的最大 token 数。None = 提供商默认值。",
        ),
        "top_p": Description(
            en="Nucleus sampling threshold (0.0-1.0). None = provider default.",
            zh="核采样阈值 (0.0-1.0)。None = 提供商默认值。",
        ),
...
class CloudflareWorkersConfig(OpenAICompatibleConfig):
    """Configuration for Cloudflare Workers AI (OpenAI-compatible endpoint).

    The base_url must contain your account ID, e.g.
    ...
    _CLOUDFLARE_DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        "base_url": Description(
            en="Cloudflare Workers AI OpenAI-compatible endpoint including your "
            "account ID, e.g. https://api.cloudflare.com/client/v4/accounts/<ACCOUNT_ID>/ai/v1",
            zh="Cloudflare Workers AI 的 OpenAI 兼容端点，需包含账户 ID，"
            "例如 https://api.cloudflare.com/client/v4/accounts/<ACCOUNT_ID>/ai/v1",
        ),
    }

    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        **OpenAICompatibleConfig.DESCRIPTIONS,
        **_CLOUDFLARE_DESCRIPTIONS,
    }
...
class StatelessLLMConfigs(I18nMixin, BaseModel):
    ...
    nvidia_nim_llm: NvidiaNIMConfig | None = Field(None, alias="nvidia_nim_llm")
