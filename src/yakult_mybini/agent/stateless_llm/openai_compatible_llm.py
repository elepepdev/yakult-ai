    ...
    async def chat_completion(
        ...
        try:
            ...
            stream: AsyncStream[
                ...
                top_p=self.top_p or NOT_GIVEN,
                presence_penalty=self.presence_penalty or NOT_GIVEN,
            )
            logger.debug(
            ...
            error_msg = "Error calling the chat endpoint: Connection error. Failed to connect to the LLM API."
            logger.error(
            ...
            error_msg = "Error calling the chat endpoint: Rate limit exceeded. Please try again later."
            logger.error(f"{error_msg} {e.response}")
            raise RuntimeError(error_msg)
            ...
            if "does not support tools" in str(e):
                ...
                return
            error_detail = str(e)
            logger.error(f"LLM API: Error occurred: {error_detail}")
            logger.info(f"Base URL: {self.base_url}")
            logger.info(f"Model: {self.model}")
            logger.info(f"Messages: {messages}")
            logger.info(f"temperature: {self.temperature}")
            raise RuntimeError(f"Error calling the chat endpoint: {error_detail}")
