...
for (let f = 0; f < 20 / dt; f++) {
  ...
  if (d > maxAbs) {
    maxAbs = d;
    worst = `t=${t.toFixed(2)}s L=${xl.toFixed(0)} R=${xr.toFixed(0)}`;
  }
  if (f % 180 === 0) console.log(`t=${t.toFixed(1)}s L ${xl.toFixed(0)}px R ${xr.toFixed(0)}px |asym| ${d.toFixed(1)}`);
}
console.log(`
avg |asym| over 20s: ${(sumAbs / n).toFixed(1)}px   max: ${maxAbs.toFixed(1)}px (${worst})`);
