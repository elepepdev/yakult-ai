...
class AccessibilityController:
    ...
    def find_elements(self, name: str = None, role: str = None, 
                      app_name: str = None, text: str = None,
                ...
                if match:
                    ...
                    if coords:
                        results.append({
                            ...
                            "states": [str(s) for s in node.get_state_set().get_states()] if node.get_state_set() else [],
                        })
    ...
    def click_element_by_name(self, name: str, role: str = None, app_name: str = None) -> Dict[str, Any]:
        elements = self.find_elements(name=name, role=role, app_name=app_name, max_results=1)
        if not elements or "error" in elements[0]:
            return {"success": False, "error": elements[0].get("error", "Element not found")}
    ...
    def list_interactive_elements(self, app_name: str = None, max_results: int = 20) -> List[Dict[str, Any]]:
        interactive_roles = ["push button", "toggle button", "check box", "radio button",
                            "menu item", "link", "slider", "spin button", "combo box",
                            "text entry", "password text", "search box"]
        
        results = []
        for role in interactive_roles:
            elements = self.find_elements(role=role, app_name=app_name, max_results=max_results)
            for el in elements:
                if "error" not in el:
                    results.append(el)
            if len(results) >= max_results:
