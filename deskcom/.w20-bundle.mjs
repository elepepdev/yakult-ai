    ...
    for (const [name, origTrack] of this.humanoidTracks.translation.entries()) {
      ...
      if (nodeName != null) {
        ...
        track.values = track.values.map(
          (v4, i) => (metaVersion === "0" && i % 3 !== 1 ? -v4 : v4) * scale
        );
      ...
      if (boneName != null) {
        ...
        if (path === "translation") {
          ...
          const trackValues = arrayChunk(origTrack.values, 3).flatMap(
            (v4) => _v3A.fromArray(v4).applyMatrix4(hipsParentWorldMatrix).toArray()
          );
          const track = origTrack.clone();
...
console.log(`camera head pos (${hp.x.toFixed(3)}, ${hp.y.toFixed(3)})`);
...
var v3 = new THREE3.Vector3();
var maxErr = 0;
for (let f = 0; f < 8.5 / dt; f++) {
  mixer.update(dt);
  if (!leftBase) {
  ...
  vrm.update(dt);
  vrm.scene.updateMatrixWorld(true);
  const wL = B.LH.getWorldPosition(v);
  const wR = B.RH.getWorldPosition(v2);
  const c = (B.LS.getWorldPosition(v3).x + B.RS.getWorldPosition(new THREE3.Vector3()).x) / 2;
  const err = Math.hypot(wR.x - (2 * c - wL.x), wR.y - wL.y, wR.z - wL.z);
  maxErr = Math.max(maxErr, err);
  const SL = wL.clone().project(cam), SR = wR.clone().project(cam);
  if (f % 120 === 0) {
    const xl = cx(SL.x), xr = cx(SR.x);
    console.log(`t=${(f * dt).toFixed(1)}s Lx=${xl.toFixed(0)} Rx=${xr.toFixed(0)} | Ldist=${(xl - 451).toFixed(0)} Rdist=${(xr - 451).toFixed(0)} | mirrorErr=${(err * 1e3).toFixed(0)}mm c=${c.toFixed(3)}`);
  }
}
console.log(`max mirrorErr over swing: ${(maxErr * 1e3).toFixed(0)}mm`);
