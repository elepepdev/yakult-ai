...
class StructuredResponseManager:
    ...
    def detect_tool_calls(self, stream_item: Union[str, Dict[str, Any]]) -> bool:
        ...
        if (
            ...
            and stream_item.get("type") == "tool_call_status"
        ):
            return True
        return False
    ...
    async def process_stream(
        ...
        async for item in stream:
            if isinstance(item, dict) and item.get("type") == "tool_call_status":
                ...
                if not phase_transitioned:
                    ...
                    if text_chunks:
                        logger.debug(
                            f"StructuredResponseManager: Yielding GREET response ({len(text_chunks)} chunks)"
                        )
                    ...
                    logger.debug(
                        "StructuredResponseManager: Transitioned to TOOL phase"
                    )
                ...
                if not phase_transitioned and isinstance(item, str):
                    ...
                    isinstance(item, dict) and item.get("type") == "final_tool_results"
                ):
                    # Tool results finalized, transition ke END phase
                    if not phase_transitioned:
                        # No tool calls, yield greeted text
                        logger.debug(
                            f"StructuredResponseManager: Yielding GREET response (final, {len(text_chunks)} chunks)"
                        )
                        for chunk in text_chunks:
                            yield chunk
                        text_chunks = []
                    ...
                    for tool_item in tool_status_queue:
                        logger.debug(
                            f"StructuredResponseManager: Yielding TOOL status - {tool_item.get('tool_name')}"
                        )
                        yield tool_item
                    tool_status_queue = []
                    ...
                    logger.debug(
                        "StructuredResponseManager: Yielding final tool results"
                    )
                    yield item
                    ...
                    if text_chunks and not phase_transitioned:
                        ...
                        logger.debug(
                            "StructuredResponseManager: Yielding GREET response before non-text item"
                        )
                        for chunk in text_chunks:
                            yield chunk
                        text_chunks = []
        ...
        if text_chunks:
            logger.debug(
                f"StructuredResponseManager: Flushing {len(text_chunks)} text chunks at stream end"
            )
            for chunk in text_chunks:
                yield chunk
        ...
        if tool_status_queue:
            logger.debug(
                f"StructuredResponseManager: Flushing {len(tool_status_queue)} tool status items at stream end"
            )
            for tool_item in tool_status_queue:
                yield tool_item
        ...
        logger.debug(
            f"StructuredResponseManager: Stream processing complete. Has tool calls: {self.has_tool_calls}"
        )
