...
class MCPClient:
    ...
    async def _ensure_server_running_and_get_session(
        ...
        server_env = server.env.copy() if server.env else {}
        if sys.platform == "linux":
            for key in ["DISPLAY", "XAUTHORITY", "XDG_RUNTIME_DIR"]:
                val = os.environ.get(key)
                if val and key not in server_env:
                    server_env[key] = val

        server_params = StdioServerParameters(
            command=server.command, args=server.args, env=server_env, cwd=server.cwd
        )
