...
class SentenceDivider:
    ...
    async def _segment_text(self, text: str) -> Tuple[List[str], str]:
        """Segment text using the configured method (offloaded to thread for pysbd/langdetect)"""
        if self.segment_method == "regex":
            return segment_text_by_regex(text)
        return await asyncio.to_thread(segment_text_by_pysbd, text)
