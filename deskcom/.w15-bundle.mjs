...
function mirrorHips(q, hipsQ) {
  const M = new THREE3.Matrix4().makeScale(-1, 1, 1);
  const hM = new THREE3.Matrix4().makeRotationFromQuaternion(hipsQ);
  const hMi = new THREE3.Matrix4().makeRotationFromQuaternion(hipsQ.clone().invert());
  const MW = new THREE3.Matrix4().multiplyMatrices(hM, M).multiply(hMi);
  const qM = new THREE3.Matrix4().makeRotationFromQuaternion(q);
  return new THREE3.Quaternion().setFromRotationMatrix(new THREE3.Matrix4().multiplyMatrices(MW, qM).multiply(MW));
}
function run(mode, seconds) {
  ...
  for (let f = 0; f < seconds / dt; f++) {
    ...
    } else if (mode === "mirrorArm") {
      E.set(0, 0, spread);
      Q.setFromEuler(E);
      B.LU.quaternion.copy(leftBase).multiply(Q);
      vrm.scene.updateMatrixWorld(true);
      const hQ = B.hips.getWorldQuaternion(new THREE3.Quaternion());
      const set = (pR, pL, qL, bone) => {
        const wQ = new THREE3.Quaternion().multiplyQuaternions(pL.getWorldQuaternion(new THREE3.Quaternion()), qL);
        bone.quaternion.copy(pR.getWorldQuaternion(new THREE3.Quaternion()).invert().multiply(mirrorHips(wQ, hQ)));
        vrm.scene.updateMatrixWorld(true);
      };
    ...
    if (mode === "mirrorArm") {
      ...
      const hx = B.hips.getWorldPosition(new THREE3.Vector3()).x;
      maxMir.push(Math.hypot(wR.x - (2 * hx - wL.x), wR.y - wL.y, wR.z - wL.z));
    }
    if (f > 0.5 / dt) {
      Ls.push(xl);
      Rs.push(xr);
  ...
  console.log(`${mode.padEnd(12)}: Lx ${stat(Ls)}  Rx ${stat(Rs)} | Ly ${ys(Ly)} Ry ${ys(Ry)}${mode === "mirrorArm" ? ` | hand err(hips) ${Math.max(...maxMir).toFixed(4)}m` : ""}`);
}
run("bakedOnly", 6.5);
run("raw", 6.5);
run("mirrorArm", 6.5);
