...
function run(mode, seconds) {
  ...
  const mirror = (a) => {
    const mn = Math.min(...a), mx = Math.max(...a);
    return `[${(451 - (mx - 451)).toFixed(0)}..${(451 - (mn - 451)).toFixed(0)}]`;
  };
  console.log(`${mode.padEnd(7)}: Lx ${stat(Lx)}  Rx ${stat(Rx)} (mirror-of-L ~ ${mirror(Lx)})`);
  console.log(`        Ly ${stat(Ly)}  Ry ${stat(Ry)}`);
}
run("raw", 8.5);
run("mirror", 8.5);
