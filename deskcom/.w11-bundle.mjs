...
function run(mode, seconds) {
  ...
  for (let f = 0; f < seconds / dt; f++) {
    ...
    if (mode === "bakedOnly") {
      ...
      const mk = (pR, pL, qL) => {
        ...
        return pR.getWorldQuaternion(new THREE3.Quaternion()).invert().multiply(mirrorW(wQ));
      };
      B.RS.quaternion.copy(mk(B.chest, B.chest, B.LS.quaternion));
  ...
  console.log(`${mode.padEnd(12)}: Lx ${stat(Ls)}  Rx ${stat(Rs)} | Ly ${ys(Ly)} Ry ${ys(Ry)}${mode === "mirrorArm" ? ` | mirror err max ${Math.max(...maxMir).toFixed(4)}m` : ""}`);
}
run("bakedOnly", 6.5);
run("raw", 6.5);
run("mirrorArm", 6.5);
