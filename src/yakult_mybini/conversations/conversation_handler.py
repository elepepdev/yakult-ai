...
async def handle_conversation_trigger(
    ...
    if msg_type == "ai-speak-signal":
        ...
        # ── ScreenMonitor: cooldown gate ────────────────────────
        if not context.screen_monitor.can_proactive_speak():
            logger.debug("Proactive speak skipped: cooldown active")
            return

        # ── Feed screen image for context (never blocks) ────────
        images = data.get("images", [])
        screen_img = next(
            (img for img in images if img.get("source") == "screen"), None
        )
        if screen_img:
            context.screen_monitor.update(screen_img.get("data", ""))

        context.screen_monitor.mark_processed()

        # Append change description for extra context
        change_desc = context.screen_monitor.change_description
        if change_desc:
            user_input = f"{user_input}\n\n{change_desc}"

        # Anti-repetition: remind the AI what it last said
        ...
        logger.info("Proactive speak triggered (cooldown passed)")
