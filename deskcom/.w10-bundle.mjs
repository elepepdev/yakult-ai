...
var cy = (y) => (y + 1) / 2 * SH;
function makeCam() {
  const cam = new THREE3.PerspectiveCamera(20, SW / SH, 0.1, 100);
  const hp = B.head.getWorldPosition(new THREE3.Vector3());
...
function mirrorW(q) {
  ...
  return new THREE3.Quaternion().setFromRotationMatrix(new THREE3.Matrix4().multiplyMatrices(M, qM).multiply(M));
}
function run(mode, seconds) {
  mixer.setTime(0);
  mixer.update(0);
  const cam = makeCam();
  let leftBase = null, rightBase = null, armSpreadTime = 0, ramp = 0;
  const Ls = [], Rs = [], Ly = [], Ry = [], dAs = [], maxMir = [];
  for (let f = 0; f < seconds / dt; f++) {
    mixer.update(dt);
    if (!leftBase) {
      leftBase = B.LU.quaternion.clone();
      rightBase = B.RU.quaternion.clone();
      ramp = 0;
    }
    armSpreadTime += Math.min(dt, 0.1);
    ramp = Math.min(1, ramp + dt / 0.5);
    const spread = (0.19 + Math.sin(armSpreadTime * Math.PI * 2 / 6) * 0.09) * ramp;
    if (mode === "bakedOnly") {
    } else if (mode === "raw") {
      E.set(0, 0, spread);
      Q.setFromEuler(E);
      ...
      E.set(0, 0, -spread);
      Q.setFromEuler(E);
      B.RU.quaternion.copy(rightBase).multiply(Q);
    } else if (mode === "worldZ") {
      vrm.scene.updateMatrixWorld(true);
      const pL = B.LS.getWorldQuaternion(new THREE3.Quaternion());
      const pR = B.RS.getWorldQuaternion(new THREE3.Quaternion());
      const rL = new THREE3.Quaternion().setFromEuler(new THREE3.Euler(0, 0, spread));
      const rR = new THREE3.Quaternion().setFromEuler(new THREE3.Euler(0, 0, -spread));
      B.LU.quaternion.copy(pL.clone().invert().multiply(rL).multiply(pL).multiply(leftBase));
      B.RU.quaternion.copy(pR.clone().invert().multiply(rR).multiply(pR).multiply(rightBase));
    } else if (mode === "mirrorChain") {
      E.set(0, 0, spread);
      Q.setFromEuler(E);
      B.LU.quaternion.copy(leftBase).multiply(Q);
      vrm.scene.updateMatrixWorld(true);
      const mk = (pR, pL, qL) => {
        ...
        const mQ = mirrorW(wQ);
        return pR.getWorldQuaternion(new THREE3.Quaternion()).invert().multiply(mQ);
      };
      B.RU.quaternion.copy(mk(B.RS, B.LS, B.LU.quaternion));
      B.RL.quaternion.copy(mk(B.RU, B.LU, B.LL.quaternion));
      B.RH.quaternion.copy(mk(B.RL, B.LL, B.LH.quaternion));
    }
    vrm.update(dt);
    vrm.scene.updateMatrixWorld(true);
    const SL = B.LH.getWorldPosition(new THREE3.Vector3()).project(cam);
    const SR = B.RH.getWorldPosition(new THREE3.Vector3()).project(cam);
    const xl = cx(SL.x), xr = cx(SR.x), yl = cy(SL.y), yr = cy(SR.y);
    if (mode === "mirrorChain") {
      const wL = B.LH.getWorldPosition(new THREE3.Vector3());
      const wR = B.RH.getWorldPosition(new THREE3.Vector3());
      maxMir.push(Math.hypot(wR.x + wL.x, wR.y - wL.y, wR.z - wL.z));
    }
    if (f > 0.5 / dt) {
      Ls.push(xl);
      Rs.push(xr);
      Ly.push(yl);
      Ry.push(yr);
      dAs.push(Math.abs(xl - 451 - (451 - xr)));
    }
  }
  const stat = (a) => {
    const mn = Math.min(...a), mx = Math.max(...a);
    return `[${mn.toFixed(0)}..${mx.toFixed(0)}] w=${(mx - mn).toFixed(0)}`;
  };
  const ys = (a) => {
    const mn = Math.min(...a), mx = Math.max(...a);
    return `y ${mn.toFixed(0)}..${mx.toFixed(0)}`;
  };
  console.log(`${mode.padEnd(12)}: Lx ${stat(Ls)}  Rx ${stat(Rs)} | L${ys(Ly)} R${ys(Ry)} | avg|asym| ${(dAs.reduce((a, b) => a + b, 0) / dAs.length).toFixed(0)}${mode === "mirrorChain" ? ` | mirror err max ${Math.max(...maxMir).toFixed(3)}m` : ""}`);
}
run("bakedOnly", 6.5);
run("raw", 6.5);
run("worldZ", 6.5);
