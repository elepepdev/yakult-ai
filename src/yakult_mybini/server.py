"""
Yakult My Bini Server
=======================
This module contains the WebSocket server for Yakult My Bini, which handles
...
class WebSocketServer:
    """
    API server for Yakult My Bini. This contains the websocket endpoint for the client, hosts the web tool, and serves static files.
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        self.app = FastAPI(title="Yakult My Bini Server")  # Added title for clarity
        self.config = config
        ...
        # Mount VRM models directory (create if not exists)
        ...
        self.app.mount(
            "/",
            CORSStaticFiles(directory="deskcom/dist/web", html=True),
            name="frontend",
        )
