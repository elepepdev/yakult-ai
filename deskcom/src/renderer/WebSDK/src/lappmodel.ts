...
export class LAppModel extends CubismUserModel {
  ...
  /**
   * 画面上のカーソル位置を設定し、モデルの視線追跡を更新する
   * @param x 正規化X座標 (-1.0 ~ 1.0)
   * @param y 正規化Y座標 (-1.0 ~ 1.0)
   */
  public setCursorPosition(x: number, y: number): void {
    this._cursorManager.set(x, y);
  }
  ...
  public update(): void {
    ...
    // カーソル追跡による変化（ドラッグより弱い影響）
    this._cursorManager.update(deltaTimeSeconds);
    this._cursorX = this._cursorManager.getX();
    this._cursorY = this._cursorManager.getY();

    // カーソルによる顔の向きの調整（ドラッグは30、カーソルは10）
    this._model.addParameterValueById(this._idParamAngleX, this._cursorX * 10);
    this._model.addParameterValueById(this._idParamAngleY, this._cursorY * 10);
    this._model.addParameterValueById(
      this._idParamAngleZ,
      this._cursorX * this._cursorY * -10
    );

    // カーソルによる体の向きの調整（ドラッグは10、カーソルは5）
    this._model.addParameterValueById(this._idParamBodyAngleX, this._cursorX * 5);

    // カーソルによる目の向きの調整（ドラッグは1.0、カーソルは0.5）
    this._model.addParameterValueById(this._idParamEyeBallX, this._cursorX * 0.5);
    this._model.addParameterValueById(this._idParamEyeBallY, this._cursorY * 0.5);
  ...
  public constructor() {
    ...
    this._cursorManager = new CubismTargetPoint();
    this._cursorX = 0.0;
    this._cursorY = 0.0;
  }
  ...
  _cursorManager: CubismTargetPoint; // カーソル追跡用（滑らかな補間）
_cursorX: number; // カーソルX位置 (-1..1)
_cursorY: number; // カーソルY位置 (-1..1)
