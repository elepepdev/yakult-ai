...
var mirrorWorld = (q) => new THREE3.Quaternion().setFromRotationMatrix(new THREE3.Matrix4().multiplyMatrices(M, new THREE3.Matrix4().makeRotationFromQuaternion(q)).multiply(M));
function makeCam() {
...
function run(mode, seconds) {
  ...
  for (let f = 0; f < seconds / dt; f++) {
    ...
    } else if (mode === "mirrorWorld") {
      E.set(0, 0, spread);
      Q.setFromEuler(E);
      B.LU.quaternion.copy(leftBase).multiply(Q);
      const lsW = B.LS.getWorldQuaternion(new THREE3.Quaternion());
      const rsW = B.RS.getWorldQuaternion(new THREE3.Quaternion());
      const q = mirrorWorld(new THREE3.Quaternion().multiplyQuaternions(lsW, B.LU.quaternion));
      B.RU.quaternion.copy(rsW.clone().invert()).multiply(q);
    }
    vrm.update(dt);
    const [xl, xr] = frame(cam, mode);
    if (f > 0.5 / dt) {
      Ls.push(xl);
      Rs.push(xr);
    }
  ...
  const stat = (a) => {
    ...
    return `[${mn.toFixed(0)}..${mx.toFixed(0)}] w=${(mx - mn).toFixed(0)}px`;
  };
  console.log(`${mode.padEnd(12)}: L ${stat(Ls)}  R ${stat(Rs)}  (center 451)`);
}
run("bakedOnly", 6.5);
run("raw", 6.5);
run("mirrorWorld", 6.5);
