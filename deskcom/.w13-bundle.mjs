    ...
    for (const [name, origTrack] of this.humanoidTracks.translation.entries()) {
      ...
      if (nodeName != null) {
        ...
        track.values = track.values.map(
          (v2, i) => (metaVersion === "0" && i % 3 !== 1 ? -v2 : v2) * scale
        );
      ...
      if (boneName != null) {
        ...
        if (path === "translation") {
          ...
          const trackValues = arrayChunk(origTrack.values, 3).flatMap(
            (v2) => _v3A.fromArray(v2).applyMatrix4(hipsParentWorldMatrix).toArray()
          );
          const track = origTrack.clone();
...
function mirrorW(q) {
  ...
  const r = new THREE3.Quaternion();
  r.setFromRotationMatrix(P);
  return r;
}
mixer.setTime(0);
mixer.update(0);
...
var wL = B.LH.getWorldPosition(v);
var c = B.hips.getWorldPosition(new THREE3.Vector3()).x;
console.log(`hips.x=${c.toFixed(3)}  lHand=(${wL.x.toFixed(3)},${wL.y.toFixed(3)},${wL.z.toFixed(3)})`);
console.log(`expected mirror of lHand about plane x=${c.toFixed(3)}: (${(2 * c - wL.x).toFixed(3)},${wL.y.toFixed(3)},${wL.z.toFixed(3)})`);
var qLU = B.LU.getWorldQuaternion(new THREE3.Quaternion());
var axis = new THREE3.Vector3(0, 1, 0).applyQuaternion(qLU);
var axisM = new THREE3.Vector3(0, 1, 0).clone();
var axisR = new THREE3.Vector3(0, 1, 0).applyQuaternion(qLU).multiplyScalar(1);
var mir = mirrorW(qLU);
var axisRef = new THREE3.Vector3(0, 1, 0).clone().applyQuaternion(mir);
console.log(`LU world +Y axis: (${axis.x.toFixed(3)},${axis.y.toFixed(3)},${axis.z.toFixed(3)})`);
console.log(`mirrored axis   : (${axisRef.x.toFixed(3)},${axisRef.y.toFixed(3)},${axisRef.z.toFixed(3)})  (expect x negated)`);
console.log(`LU world quat: x=${qLU.x.toFixed(3)} y=${qLU.y.toFixed(3)} z=${qLU.z.toFixed(3)} w=${qLU.w.toFixed(3)}`);
console.log(`mirrored quat: x=${mir.x.toFixed(3)} y=${mir.y.toFixed(3)} z=${mir.z.toFixed(3)} w=${mir.w.toFixed(3)}`);
console.log(`RU world quat: x=${B.RU.getWorldQuaternion(new THREE3.Quaternion()).x.toFixed(3)} y=${B.RU.getWorldQuaternion(new THREE3.Quaternion()).y.toFixed(3)} z=${B.RU.getWorldQuaternion(new THREE3.Quaternion()).z.toFixed(3)} w=${B.RU.getWorldQuaternion(new THREE3.Quaternion()).w.toFixed(3)}`);
